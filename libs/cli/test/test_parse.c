...
spec(parse) {
    ...
    it("fails omitting the required options") {
    ...
    it("supports both short and long forms for flags") {
        ...
        parse_check_success(_testCtx, invoc);
        anvil(cli_parse_provided(invoc, flagA));
        anvil(cli_parse_provided(invoc, flagC));
        anvil(cli_parse_provided(invoc, flagD));
        cli_parse_destroy(invoc);
    }
    ...
    it("supports passing multiple short flags in a single block") {
        ...
        parse_check_success(_testCtx, invoc);
        anvil(cli_parse_provided(invoc, flagB));
        anvil(cli_parse_provided(invoc, flagC));
        cli_parse_destroy(invoc);
    }
    ...
    it("supports value flags with multiple values ass seperate strings") {
        CliInvocation* invoc = cli_parse(
            app, 7, (const char*[]){"--flag-e-multival", "Hello", "Beautifull", "World", "-a", "Hello", "ArgVal"});
        parse_check_values(
            _testCtx,
            invoc,
            flagE,
    ...
    it("supports value flags with multiple values as a single string") {
        CliInvocation* invoc  = cli_parse(app, 5, (const char*[]){"--flag-e-multival", "Hello,Beautiful,World", "-a", "Hello", "ArgVal"});
        parse_check_values(_testCtx, invoc, flagE, (String[]){string_lit("Hello"), string_lit("Beautiful"), string_lit("World")}, 3);
        cli_parse_destroy(invoc);
    }

    it("supports value flags with multiple values as a mix of single and multiple strings") {
        CliInvocation* invoc = cli_parse(app, 7, (const char*[]){"--flag-e-multival", "A,,,B", "C,,", "D,E", "-a", "Hello", "ArgVal"});
        parse_check_values(_testCtx, invoc, flagE, (String[]){string_lit("A"), string_lit("B"), string_lit("C"), string_lit("D"), string_lit("E")}, 5);
        cli_parse_destroy(invoc);
    }

    it("supports arguments with multiple values as seperate strings") {
        CliInvocation* invoc =
    ...
    it("supports arguments with multiple values as a single string") {
        CliInvocation* invoc = cli_parse(app, 4, (const char*[]){"-a", "Hello", "ArgVal", "Hello,Beautiful,World"});
        parse_check_values(_testCtx, invoc, argB, (String[]){string_lit("Hello"), string_lit("Beautiful"), string_lit("World")}, 3);
        cli_parse_destroy(invoc);
    }

    it("supports arguments with multiple values as a mix of single and multiple strings") {
        CliInvocation* invoc = cli_parse(app, 6, (const char*[]){"-a", "Hello", "ArgVal", ",A,B,", "C", "D,E"});
        parse_check_values(_testCtx, invoc, argB, (String[]){string_lit("A"), string_lit("B"), string_lit("C"), string_lit("D"), string_lit("E")}, 5);
        cli_parse_destroy(invoc);
    }
    ...
    it("supports single dash for terminating a list of values") {
        CliInvocation* invoc = cli_parse(
            app,
            9,
            (const char*[]){"-a", "Hello", "ArgVal", "--flag-e-multival", "Some", "Values", "-", "Hello", "World"});
        parse_check_values(
    ...
    it("ignores empty values") {
        ...
        parse_check_values(
            _testCtx, invoc, argB, (String[]){string_lit("1337"), string_lit("42")}, 2);
        cli_parse_destroy(invoc);
    }

    it("fails when passing the same flag twice in short form") {
        CliInvocation* invoc =
            cli_parse(app, 5, (const char*[]){"-a", "Hello", "-a", "World", "42"});
    ...
    it("fails when passing the same flag twice in long form") {
        CliInvocation* invoc = cli_parse(
            app, 5, (const char*[]){"--flag-a-req", "Hello", "--flag-a-req", "World", "42"});
    ...
    it("fails when violating an exclusion") {
        CliInvocation* invoc =
            cli_parse(app, 7, (const char*[]){"-d", "42", "--flag-e-multival", "B", "-a", "Hello", "ArgVal"});
        parse_check_fail(
            _testCtx,
            invoc,
            (String[]){
    ...
    it("succeeds when providing an alternative instead of a required option") {
        CliInvocation* invoc = cli_parse(app, 2, (const char*[]) {
            "ArgVal", "--flag-g"
        });
