...
spec(read) {
    ...
    it("returns the provided string") {
        ...
        anvil_eq_string(cli_read_string(invoc, flag, string_lit("Backup")), string_lit("Hello World"));
    ...
    it("returns the default when not providing a string") {
        ...
        anvil_eq_string(cli_read_string(invoc, flag, string_lit("Goodbye")), string_lit("Goodbye"));
    ...
    it("returns the default when not providing a f64") {
        ...
        CliInvocation* invoc = cli_parse(app, 0, null);
        anvil_eq_float(cli_read_f64(invoc, flag, 999.999), 999.999, 1e-32);
    ...
    it("returns the index of the provided choice string") {
        ...
        CliInvocation* invocA = cli_parse(app, 2, (const char*[]) {
            "-c",
            "choiceA"
        });
        anvil_eq_int(cli_read_choice_array(invocA, flag, choices, 999), 0);
        cli_parse_destroy(invocA);

        CliInvocation* invocB = cli_parse(app, 2, (const char*[]) {
                    "-c",
                    "choiceB"
                });
        anvil_eq_int(cli_read_choice_array(invocB, flag, choices, 999), 1);
        cli_parse_destroy(invocB);
    }

    it("returns the deafult when not providing a choice string") {
        static const String choices[] = {
            string_static("choiceA"),
            string_static("choiceB"),
        };
        ...
        CliInvocation* invoc = cli_parse(app, 0, null);
        anvil_eq_int(cli_read_choice_array(invoc, flag, choices, 999), 999);
