...
spec(sink_pretty) {
    ...
    setup() {
        ...
        log_add_sink(logger, log_sink_pretty(g_alloc_heap, tmpFile, mask, LogSinkPrettyFlags_DestroyFile));
    ...
    it("supports plain log messages without parameters") {
        ...
        anvil(string_match_glob(dynstring_view(&buffer), string_lit("* [INFO] Hello World\n"), StringMatchFlags_None));
    }
    ...
    it("aligns parameter values") {
        log(logger, LogLevel_Info, "Hello World", log_param("short", fmt_int(42)), log_param("very-very-long", fmt_text_lit("Hi!")), log_param("shorter", fmt_duration(time_second)));

        file_seek_sync(tmpFile, 0);
        file_read_to_end_sync(tmpFile, &buffer);

        anvil(string_match_glob(dynstring_view(&buffer), string_lit(
            "* [INFO] Hello World\n"
            "  short:          42\n"
            "  very-very-long: Hi!\n"
            "  shorter:        1s\n"),
    ...
    it("supports list parameters") {
        log(logger, LogLevel_Info, "Hello World", log_param("param", fmt_list_lit(fmt_int(1), fmt_int(2), fmt_int(3))));
    ...
    it("ignores messages which do not match the mask") {
        ...
        anvil_eq_string(dynstring_view(&buffer), string_empty);
    }
