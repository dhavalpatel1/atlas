...
_Static_assert(array_elems(g_level_strs) == LogLevel_Count, "Incorrect number of Log Level strings");
...
void log_append(Logger* logger, LogLevel level, SourceLoc loc, String str, const LogParam* params) {
    diag_assert_msg(logger, "Logger not initialized");
    ...
    dynarray_for_t(&logger->sinks, LogSink*, sink, {
        (*sink)->write(*sink, level, loc, timestamp, message, params);
    });
}
