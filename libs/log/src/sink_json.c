...
static JsonVal log_to_json(JsonDoc* doc, const FormatArg* arg) {
    switch (arg->type) {
        ...
        case FormatArgType_f64: {
            return json_add_number(doc, (f64)arg->value_f64);
        }
...
static void log_sink_json_write(LogSink* sink, LogLevel level, SourceLoc srcLoc, TimeReal timestamp, String message, const LogParam* params) {
    LogSinkJson* jsonSink = (LogSinkJson*)sink;
    if (!log_mask_enabled(jsonSink->mask, level)) {
        return;
    }
    ...
    json_add_field_str(doc, root, string_lit("level"), json_add_string(doc, log_level_str(level)));
    json_add_field_str(doc, root, string_lit("timestamp"), json_add_string(doc, format_write_arg_scratch(&fmt_time(timestamp))));
    json_add_field_str(doc, root, string_lit("file"), json_add_string(doc, format_write_arg_scratch(&fmt_path(srcLoc.file))));
...
LogSink* log_sink_json(Allocator* alloc, File* file, const LogMask mask, const LogSinkJsonFlags flags) {
...
LogSink* log_sink_json_default(Allocator* alloc, const LogMask mask) {
    const String logPath = path_build_scratch(path_parent(g_path_executable), string_lit("logs"), path_name_timestamp_scratch(path_stem(g_path_executable), string_lit("log")));
