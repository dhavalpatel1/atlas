...
static FormatArg arg_style_loglevel(LogSinkPretty* sink, LogLevel level) {
    if (sink->style) {
        switch (level) {
            case LogLevel_Debug: {
...
static void log_sink_pretty_write(LogSink* sink, LogLevel level, SourceLoc srcLoc, TimeReal timestamp, String message, const LogParam* params) {
    (void)srcLoc;
    LogSinkPretty* prettySink = (LogSinkPretty*)sink;
    if (!log_mask_enabled(prettySink->mask, level)) {
        return;
    }
    ...
    fmt_write(&str, "{}{} {}{}[{}] {}{}\n",
        ...
        arg_style_loglevel(prettySink, level),
        fmt_text(log_level_str(level)),
        arg_style_reset(prettySink),
        fmt_text(message)
    );
...
LogSink* log_sink_pretty(Allocator* alloc, File* file, const LogMask mask, const LogSinkPrettyFlags flags) {
...
LogSink* log_sink_pretty_default(Allocator* alloc, LogMask mask) {
