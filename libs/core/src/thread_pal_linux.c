...
void thread_pal_set_name(const String str) {
    ...
    if (str.size > maxNameLen) {
        diag_assert_fail("Thread name '{}' is too long, maximum is {} chars", fmt_text(str), fmt_int(maxNameLen));
    }
...
FORCE_INLINE bool thread_pal_atomic_compare_exchange_i64(i64 *ptr, i64 *expected, i64 value) {
...
void thread_pal_join(ThreadHandle handle) {
    void* returnData;
    const int res = pthread_join((pthread_t)handle, &returnData);
...
void thread_pal_sleep(const TimeDuration duration) {
    ...
    while ((res = nanosleep(&ts, &ts)) == -1 && errno == EINTR);
...
ThreadCondition thread_pal_cond_create(Allocator *allocator) {
    ThreadConditionData* data = alloc_alloc_t(allocator, ThreadConditionData);
...
void thread_pal_cond_destroy(ThreadCondition cond) {
    ThreadConditionData* data = (ThreadConditionData*)cond;
...
void thread_pal_cond_wait(ThreadCondition cond, ThreadMutex mutex) {
    ThreadConditionData* condData = (ThreadConditionData*)cond;
