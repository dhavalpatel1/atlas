...
FORCE_INLINE usize bitset_size(BitSet bits) {
    return bytes_to_bits(bits.size);
}

FORCE_INLINE bool bitset_test(BitSet bits, usize idx) {
...
FORCE_INLINE void bitset_set(BitSet bits, usize idx) {
    diag_assert(idx < bitset_size(bits));
    *mem_at_u8(bits, bits_to_bytes(idx)) |= 1u << bit_in_byte(idx);
}

FORCE_INLINE void bitset_clear(BitSet bits, usize idx) {
    diag_assert(idx < bitset_size(bits));
    *mem_at_u8(bits, bits_to_bytes(idx)) &= ~(1u << bit_in_byte(idx));
}

void bitset_clear_all(BitSet bits) {
    mem_set(bits, 0);
}
