...
static void test_file_verify_data(AnvilTestContext* _testCtx, String input) {
    for (usize i = 0; i != input.size; ++i) {
        anvil_eq_int(*string_at(input, i), i % 255);
    }
}
...
spec(file) {
    ...
    it("can create a new file by opening a file-handle with 'Create' mode") {
        ...
        anvil_eq_int(file_create(g_alloc_heap, path, FileMode_Open, FileAccess_Read, &file), FileResult_Success);
    ...
    it("can create a new directory") {
        ...
        File* dirHandle;
        anvil_eq_int(file_create(g_alloc_scratch, path, FileMode_Open, FileAccess_None, &dirHandle), FileResult_Success);
        if (dirHandle) {
            file_destroy(dirHandle);
        }

        anvil_eq_int(file_delete_dir_sync(path), FileResult_Success);
    }
