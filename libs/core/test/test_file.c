...
spec(file) {
    it("can read an write file contents") {
        ...
        File* file;
        anvil_eq_int(file_temp(alloc, &file), FileResult_Success);
        ...
        anvil_eq_int(file_read_sync(file, &str), FileResult_Success);
        anvil_eq_string(dynstring_view(&str), string_lit("Hello World!"));
