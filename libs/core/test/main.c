...
static int run_tests(const bool outputPassingTests) {
    ...
    register_spec(anvil, alloc_bump);
    register_spec(anvil, alloc_page);
    register_spec(anvil, alloc_scratch);
    register_spec(anvil, ascii);
    register_spec(anvil, base64);
    register_spec(anvil, bits);
    register_spec(anvil, bitset);
    register_spec(anvil, compare);
    ...
    register_spec(anvil, math);
    register_spec(anvil, memory);
    register_spec(anvil, path);
    register_spec(anvil, rng);
    register_spec(anvil, shuffle);
    register_spec(anvil, sort);
    register_spec(anvil, string);
    register_spec(anvil, thread);
    register_spec(anvil, time);
    register_spec(anvil, utf8);
    register_spec(anvil, winutils);
