...
spec(thread) {
    ...
    it("can store and load integers atomically") {
        ...
        thread_join(exec);
        anvil_eq_u64(thread_atomic_load_i64(&value), 1337);
    }
    ...
    it("can compare and exchange integers atomically") {
        ...
        for (i32 i = 0; i != 1000; ++i) {
            ...
            if (!thread_atomic_compare_exchange_i64(&value, &expected, 42)) {
                anvil_eq_u64(expected, 42);
            }
        }
        thread_join(exec);
        anvil(thread_atomic_load_i64(&value) == 1337 || thread_atomic_load_i64(&value) == 42);
    }
    ...
    it("can add integers atomically") {
        ...
        thread_join(exec);
        anvil_eq_u64(thread_atomic_load_i64(&value), 20000);
    }
    ...
    it("can substract integers atomically") {
        ...
        thread_join(exec);
        anvil_eq_u64(thread_atomic_load_i64(&value), 0);
    }
    ...
    it("can trylock a mutex when its currently unlocked") {
        ...
        anvil(thread_mutex_trylock(mutex));
        thread_mutex_unlock(mutex);
