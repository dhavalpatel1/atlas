...
spec(parse) {
    ...
    it("can parse arrays") {
        ...
        for (usize i = 0; i != array_elems(data); ++i) {
            ...
            anvil(json_eq(doc, res.val, data[i].expected));
        }
    }
    ...
    it("can parse strings") {
        ...
        for (usize i = 0; i != array_elems(data); ++i) {
            ...
            anvil(string_is_empty(rem));
            anvil_require(res.type == JsonResultType_Success);
            anvil_eq_string(json_string(doc, res.val), data[i].expected);
        }
    }
    ...
    it("can parse booleans") {
        ...
        for (usize i = 0; i != array_elems(data); ++i) {
            ...
            anvil(string_is_empty(rem));
            anvil_require(res.type == JsonResultType_Success);
            anvil(json_bool(doc, res.val) == data[i].expected);
        }
    }
    ...
    it("fails on invalid input") {
        ...
        for (usize i = 0; i != array_elems(data); ++i) {
            ...
            anvil_require(res.type == JsonResultType_Fail);
            anvil_eq_string(json_error_str(res.error), data[i].error);
        }
    }
    ...
    it("fails when input contains a too long string") {
        ...
        anvil_require(res.type == JsonResultType_Fail);
        anvil_eq_string(json_error_str(res.error), string_lit("TooLongString"));
    }
    ...
    it("fails when input contains too deep nesting") {
        ...
        anvil_require(res.type == JsonResultType_Fail);
        anvil_eq_string(json_error_str(res.error), string_lit("MaximumDepthExceeded"));
    }
