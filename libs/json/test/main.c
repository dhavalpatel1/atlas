...
static int run_tests(const bool outputPassingTests) {
    ...
    register_spec(anvil, doc);
    register_spec(anvil, eq);
    register_spec(anvil, parse);
    register_spec(anvil, write);

    const AnvilRunFlags flags =
        outputPassingTests ? AnvilRunFlags_OutputPassingTests : AnvilRunFlags_None;
