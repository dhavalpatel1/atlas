...
spec(doc) {
    ...
    it("returns false when adding two fields with the same name to an object") {
        ...
        anvil(json_add_field_str(doc, val, string_lit("a"), json_add_null(doc)));
        anvil(!json_add_field_str(doc, val, string_lit("a"), json_add_null(doc)));
        anvil(!json_add_field_str(doc, val, string_lit("a"), json_add_number(doc, 42)));
        anvil(json_add_field_str(doc, val, string_lit("b"), json_add_null(doc)));
    }
